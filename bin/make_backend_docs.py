...
def alter_doc(backend):
    ...
    doc_file = f"docs/content/{backend}.md"
    if not os.path.exists(doc_file):
        raise ValueError(f"Didn't find doc file {doc_file}")
    new_file = f"{doc_file}~new~"
    altered = False
    with (open(doc_file, "r", encoding="utf_8") as in_file, open(new_file, "w", encoding="utf_8") as out_file):
        ...
        for line in in_file:
            ...
            if stop in line:
                if in_docs:
                    in_docs = False
    os.rename(doc_file, f"{doc_file}~")
    os.rename(new_file, doc_file)
    if not altered:
        raise ValueError(f"Didn't find '{start}' markers for in {doc_file}")
...
def main(args):
    ...
    if (len(args) == 2):
        try:
            alter_doc(args[1])
            print(f"Added docs for {args[1]} backend")
        except Exception as e:
            print(f"Failed adding docs for {args[1]} backend: {e}")
        return
    ...
    for backend in find_backends():
        try:
            ...
            print(f"Failed adding docs for {backend} backend: {e}")
            failed += 1
        else:
            success += 1
